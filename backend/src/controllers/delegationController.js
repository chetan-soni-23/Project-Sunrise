  ...
  try {
    ...
    if (error.code === '23505') {
      ...
      if (error.constraint === 'uq_approvals_booking_approver') {
        return res.status(400).json({ error: 'Transfer failed: delegate already has an approval for one or more bookings. Please resolve conflicts manually.' });
      }
      return res.status(400).json({ error: 'Active delegation to this user already exists' });
    }
...
const updateDelegation = async (req, res) => {
  try {
    ...
    const oldDelegateId = existing.rows[0].delegated_to_id;
